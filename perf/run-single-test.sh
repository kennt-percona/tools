#!/bin/bash
#
# Runs through a single (full test run, all client thread variations)
#
...
test_dir="single-${test_type}-${ssl_or_nossl}-${applier_count}-${table_count}"
mkdir -p $test_dir
cd $test_dir
../run-timer.sh &
timer_pid=$!
...
./run-test.sh $test_dir index $table_count
kill $timer_pid
timer_pid=0
