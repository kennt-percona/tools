...
#
# This script creates the config for a PXC cluster
# that uses a vault server.  This will also create
# an async master node
#
# This assumes that the vault server is already running separately.
# 
...
if (( "$#" != 2 )); then
  echo ""
  echo "Usage:  pxc-vault.sh <config-file> <ipaddr>"
  echo ""
